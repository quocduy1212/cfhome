...
const TopMenu = ({ className = '', page, globalSettingsChange }) => (
  <nav className={`${className} mt3 h2`}>
    <span
      className={`mr4 fr dib pointer ${page === 'ahihi' ? activeModeClass : ''}`}
      onClick={() => globalSettingsChange({ page: 'ahihi' })}
    >
      ahihi
    </span>
    <span
      className={`mr4 fr dib pointer ${page === 'bookmarks' ? activeModeClass : ''}`}
      onClick={() => globalSettingsChange({ page: 'bookmarks' })}
    >
      bookmarks
    </span>
    <span
      className={`mr4 fr dib pointer ${page === 'indicators' ? activeModeClass : ''}`}
